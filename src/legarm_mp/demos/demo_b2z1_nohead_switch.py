...
def main(argv):
    ...
    pd_control_duration = 2.0  # PD 控制持续时间 [s]
    ...
    # gripper_traj_planner.plan_cartesian_motion(
    #     gripper_pos_0, gripper_orn_0, gripper_pos_0, gripper_orn_0, 8.*scale)  # 保持不动
    ...
    csv_file = csv.writer(open(rootdir + "/data/b2z1_nohead_switch.csv", "w"))
...
###############################################################################################
    ...
    while current_time - start_time < scaled_duration:
        ...
        step = min(step, len(body_traj_planner.traj_lin_vel_lcl) - 1)
